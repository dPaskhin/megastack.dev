...
</code></pre><p>Pass <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.ExecutorManagerOptions.html#keyidgenerator"><code>keyIdGenerator</code> <sup>↗</sup></a> option to the <code>ExecutorManager</code> constructor to change the way key identity is computed. The returned key ID can be anything, a string, or an object.</p><p>If you're using objects as executor keys, then you may want to use stable serialized form of executor keys as key IDs (when object keys are sorted alphabetically during serialization). In this case use any library that supports stable JSON serialization:</p><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-smi">JSONMarshal</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>json-marshal<span class="pl-pds">'</span></span>;

<span class="pl-k">const</span> <span class="pl-c1">manager</span> <span class="pl-k">=</span> <span class="pl-k">new</span> <span class="pl-en">ExecutorManager</span>({
  <span class="pl-en">keyIdGenerator</span>: <span class="pl-v">key</span> <span class="pl-k">=></span> <span class="pl-smi">JSONMarshal</span>.<span class="pl-en">stringify</span>(<span class="pl-smi">key</span>, { isStable: <span class="pl-c1">true</span> }),
});

<span class="pl-k">const</span> <span class="pl-c1">rookyExecutor</span> <span class="pl-k">=</span> <span class="pl-smi">manager</span>.<span class="pl-en">getOrCreate</span>({ id: <span class="pl-c1">123</span>, name: <span class="pl-s"><span class="pl-pds">'</span>Rooky<span class="pl-pds">'</span></span> });
<span class="pl-c">// ⮕ Executor&#x3C;any></span>

<span class="pl-c">// 🟡 Key properties are listed in a different order</span>
<span class="pl-smi">manager</span>.<span class="pl-c1">get</span>({ name: <span class="pl-s"><span class="pl-pds">'</span>Rooky<span class="pl-pds">'</span></span>, id: <span class="pl-c1">123</span> });
<span class="pl-c">// ⮕ rookyExecutor</span>
</code></pre><div class="markdown-alert markdown-alert-tip"><p class="markdown-alert-title"><svg class="octicon octicon-light-bulb mr-2" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="M8 1.5c-2.363 0-4 1.69-4 3.75 0 .984.424 1.625.984 2.304l.214.253c.223.264.47.556.673.848.284.411.537.896.621 1.49a.75.75 0 0 1-1.484.211c-.04-.282-.163-.547-.37-.847a8.456 8.456 0 0 0-.542-.68c-.084-.1-.173-.205-.268-.32C3.201 7.75 2.5 6.766 2.5 5.25 2.5 2.31 4.863 0 8 0s5.5 2.31 5.5 5.25c0 1.516-.701 2.5-1.328 3.259-.095.115-.184.22-.268.319-.207.245-.383.453-.541.681-.208.3-.33.565-.37.847a.751.751 0 0 1-1.485-.212c.084-.593.337-1.078.621-1.489.203-.292.45-.584.673-.848.075-.088.147-.173.213-.253.561-.679.985-1.32.985-2.304 0-2.06-1.637-3.75-4-3.75ZM5.75 12h4.5a.75.75 0 0 1 0 1.5h-4.5a.75.75 0 0 1 0-1.5ZM6 15.25a.75.75 0 0 1 .75-.75h2.5a.75.75 0 0 1 0 1.5h-2.5a.75.75 0 0 1-.75-.75Z"></path></svg>Tip</p><p>With additional configuration, <a href="https://github.com/smikhalevski/json-marshal#readme">json-marshal <sup>↗</sup></a> can stringify and parse any data structure.</p></div><p>If you want to use object references as executor key IDs, provide an identity function:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">manager</span> <span class="pl-k">=</span> <span class="pl-k">new</span> <span class="pl-en">ExecutorManager</span>({
  <span class="pl-en">keyIdGenerator</span>: <span class="pl-v">key</span> <span class="pl-k">=></span> <span class="pl-smi">key</span>,
});

<span class="pl-k">const</span> <span class="pl-c1">rookyKey</span> <span class="pl-k">=</span> { id: <span class="pl-c1">123</span> };

<span class="pl-k">const</span> <span class="pl-c1">rookyExecutor</span> <span class="pl-k">=</span> <span class="pl-smi">manager</span>.<span class="pl-en">getOrCreate</span>(<span class="pl-smi">rookyKey</span>);

<span class="pl-c">// 🟡 Executors are different because different objects were used as keys</span>
<span class="pl-smi">manager</span>.<span class="pl-c1">get</span>(<span class="pl-smi">rookyKey</span>) <span class="pl-k">!==</span> <span class="pl-smi">manager</span>.<span class="pl-en">getOrCreate</span>({ id: <span class="pl-c1">123</span> });
...
</code></pre><p>By default, <code>abortWhen</code> only aborts the currently pending task. Here's how every newly executed task can be instantly aborted if the last value emitted by the observable was <code>true</code> and a delay has ran out:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">executor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-smi">heavyTask</span>, [
  <span class="pl-en">abortWhen</span>(<span class="pl-smi">navigatorOffline</span>, {
    delay: <span class="pl-c1">5_000</span>,

    <span class="pl-c">// 🟡 Abort every newly executed task</span>
    isSustained: <span class="pl-c1">true</span>,
  }),
]);
</code></pre><p>Read more about observables in the <a href="#invalidatewhen"><code>InvalidateWhen</code></a> section.</p><h2 id="detachdeactivated"><a class="markdown-permalink" href="#detachdeactivated"><span class="icon icon-link"></span></a><code>detachDeactivated</code></h2><p><a href="#detach-an-executor">Detaches the executor</a> after the timeout if the executor is deactivated.</p><!-- prettier-ignore --><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-smi">detachDeactivated</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor/plugin/detachDeactivated<span class="pl-pds">'</span></span>;
...
<span class="pl-k">const</span> <span class="pl-c1">executor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-c1">42</span>, [
  <span class="pl-en">detachInactive</span>({ delayBeforeActivated: <span class="pl-c1">5_000</span> }),
]);
...
<span class="pl-c">// breadExecutor is invalidated</span>
<span class="pl-smi">cheeseExecutor</span>.<span class="pl-c1">resolve</span>(<span class="pl-s"><span class="pl-pds">'</span>Mozzarella<span class="pl-pds">'</span></span>);
</code></pre><h2 id="invalidatewhen"><a class="markdown-permalink" href="#invalidatewhen"><span class="icon icon-link"></span></a><code>invalidateWhen</code></h2><p><a href="#invalidate-results">Invalidates</a> the settled executor result when the <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.Observable.html">observable <sup>↗</sup></a> emits <code>true</code>.</p><p>For example, if the window was offline for more than 5 seconds, then the executor would be invalidated when the window goes is back online:</p><!-- prettier-ignore --><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-smi">invalidateWhen</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor/plugin/invalidateWhen<span class="pl-pds">'</span></span>;
...
<span class="pl-k">const</span> <span class="pl-c1">executor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-smi">heavyTask</span>, [
  <span class="pl-en">invalidateWhen</span>(<span class="pl-smi">navigatorOnline</span>, { delay: <span class="pl-c1">5_000</span> }),
]);
...
<span class="pl-k">import</span> <span class="pl-smi">invalidateWhen</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor/observable/invalidateWhen<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> <span class="pl-smi">navigatorOffline</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor/observable/navigatorOffline<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> <span class="pl-smi">navigatorOnline</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor/observable/navigatorOnline<span class="pl-pds">'</span></span>;
...
<span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-smi">heavyTask</span>, [
  ...
  <span class="pl-c">// Retry the task every 5 seconds if if succeeds</span>
  <span class="pl-en">retryFulfilled</span>({ delay: <span class="pl-c1">5_000</span> }),

  <span class="pl-c">// Abort the task if the window looses focus for at least 10 seconds</span>
  <span class="pl-en">abortWhen</span>(<span class="pl-smi">windowBlurred</span>, { delay: <span class="pl-c1">10_000</span> }),

  <span class="pl-c">// Abort the pending task if the device is disconnected from the network</span>
  <span class="pl-en">abortWhen</span>(<span class="pl-smi">navigatorOffline</span>),

  <span class="pl-c">// Invalidate results when the window was blurred and then gains focus</span>
  <span class="pl-en">invalidateWhen</span>(<span class="pl-smi">windowFocused</span>),

  <span class="pl-c">// Invalidate results if the device was disconnected from the network</span>
  <span class="pl-c">// and then connects again</span>
  <span class="pl-en">invalidateWhen</span>(<span class="pl-smi">navigatorOnline</span>),
]);
...
<span class="pl-smi">executor</span>.<span class="pl-en">activate</span>();
  ...
  <span class="pl-en">retryActivated</span>({ delay: <span class="pl-c1">5_000</span> }),
]);
...
</code></pre><p>If the task fails, is aborted, or if an executor is deactivated then the plugin stops the retry process.</p><p>With the default configuration, the plugin would infinitely retry the task of an <a href="#activate-an-executor">active</a> executor with a 5-second delay between retries. This is effectively a decent polling strategy that kicks in only if someone is actually using an executor.</p><p>Specify the number of times the task should be re-executed if it succeeds:</p><pre><code class="language-ts"><span class="pl-en">retryFulfilled</span>({ count: <span class="pl-c1">3</span> });
...
</code></pre><p>Use <code>retryFulfilled</code> in conjunction with <a href="#retryrejected"><code>retryRejected</code></a> to create a polling sequence. For example, this configuration would infinitely poll the <code>heavyTask</code> and would retry failed iterations with an exponential backoff and no more then 3 times in a row:</p><!-- prettier-ignore --><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">executor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-smi">heavyTask</span>, [
  <span class="pl-en">retryFulfilled</span>(),
  <span class="pl-en">retryRejected</span>({ count: <span class="pl-c1">5</span> }),
]);
</code></pre><p>You can also use a combination of <a href="#invalidateafter"><code>invalidateAfter</code></a> and <a href="#retryinvalidated"><code>retryInvalidated</code></a> to create an infinite polling regardless of the result of the previous iteration:</p><!-- prettier-ignore --><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">executor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-smi">heavyTask</span>, [
  <span class="pl-en">invalidateAfter</span>(<span class="pl-c1">5_000</span>),
  <span class="pl-en">retryInvalidated</span>(),
]);
...
</code></pre><h2 id="syncbrowserstorage"><a class="markdown-permalink" href="#syncbrowserstorage"><span class="icon icon-link"></span></a><code>syncBrowserStorage</code></h2><p>Synchronizes the executor state with <code>localStorage</code> or <code>sessionStorage</code> item. No-op in a non-browser environment.</p><pre><code class="language-ts"><span class="pl-k">import</span> <span class="pl-smi">syncBrowserStorage</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor/plugin/syncBrowserStorage<span class="pl-pds">'</span></span>;

<span class="pl-k">const</span> <span class="pl-c1">executor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-c1">42</span>, [<span class="pl-en">syncBrowserStorage</span>()]);
...
<span class="pl-k">const</span> <span class="pl-c1">executor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-c1">42</span>, [
  <span class="pl-en">syncBrowserStorage</span>({
    serializer: <span class="pl-smi">JSONMarshal</span>,
  }),
]);
</code></pre><div class="markdown-alert markdown-alert-tip"><p class="markdown-alert-title"><svg class="octicon octicon-light-bulb mr-2" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="M8 1.5c-2.363 0-4 1.69-4 3.75 0 .984.424 1.625.984 2.304l.214.253c.223.264.47.556.673.848.284.411.537.896.621 1.49a.75.75 0 0 1-1.484.211c-.04-.282-.163-.547-.37-.847a8.456 8.456 0 0 0-.542-.68c-.084-.1-.173-.205-.268-.32C3.201 7.75 2.5 6.766 2.5 5.25 2.5 2.31 4.863 0 8 0s5.5 2.31 5.5 5.25c0 1.516-.701 2.5-1.328 3.259-.095.115-.184.22-.268.319-.207.245-.383.453-.541.681-.208.3-.33.565-.37.847a.751.751 0 0 1-1.485-.212c.084-.593.337-1.078.621-1.489.203-.292.45-.584.673-.848.075-.088.147-.173.213-.253.561-.679.985-1.32.985-2.304 0-2.06-1.637-3.75-4-3.75ZM5.75 12h4.5a.75.75 0 0 1 0 1.5h-4.5a.75.75 0 0 1 0-1.5ZM6 15.25a.75.75 0 0 1 .75-.75h2.5a.75.75 0 0 1 0 1.5h-2.5a.75.75 0 0 1-.75-.75Z"></path></svg>Tip</p><p>With additional configuration, <a href="https://github.com/smikhalevski/json-marshal#readme">json-marshal <sup>↗</sup></a> can stringify and parse any data structure.</p></div><p>By default, <code>syncBrowserStorage</code> plugin uses a <a href="#executor-keys">serialized executor key</a> as a storage key. You can provide a custom key via <a href="https://smikhalevski.github.io/react-executor/interfaces/plugin_syncBrowserStorage.SyncBrowserStorageOptions.html#storagekey"><code>storageKey</code> <sup>↗</sup></a> option:</p><pre><code class="language-ts"><span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-c1">42</span>, [<span class="pl-en">syncBrowserStorage</span>({ storageKey: <span class="pl-s"><span class="pl-pds">'</span>hello<span class="pl-pds">'</span></span> })]);
</code></pre><h2 id="syncexternalstore"><a class="markdown-permalink" href="#syncexternalstore"><span class="icon icon-link"></span></a><code>syncExternalStore</code></h2><p>Persists the executor state in the observable external store.</p><p>Here's the minimal external store example:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">useExecutor</span>, <span class="pl-smi">ExecutorState</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> <span class="pl-smi">syncExternalStore</span>, { <span class="pl-smi">ExternalStore</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor/plugin/syncExternalStore<span class="pl-pds">'</span></span>;

<span class="pl-k">let</span> <span class="pl-smi">myStoredState</span><span class="pl-k">:</span> <span class="pl-en">ExecutorState</span> <span class="pl-k">|</span> <span class="pl-c1">null</span> <span class="pl-k">=</span> <span class="pl-c1">null</span>;

<span class="pl-k">const</span> <span class="pl-c1">myStore</span><span class="pl-k">:</span> <span class="pl-en">ExternalStore</span>&#x3C;<span class="pl-en">ExecutorState</span>> <span class="pl-k">=</span> {
  <span class="pl-en">get</span>() {
    <span class="pl-k">return</span> <span class="pl-smi">myStoredState</span>;
  },

  <span class="pl-en">subscribe</span>(<span class="pl-v">listener</span>) {
    <span class="pl-c">// Place subscription login here</span>
    <span class="pl-k">return</span> () <span class="pl-k">=></span> {};
  },
};

<span class="pl-k">const</span> <span class="pl-c1">myExecutor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>test<span class="pl-pds">'</span></span>, <span class="pl-c1">42</span>, [<span class="pl-en">syncExternalStore</span>(<span class="pl-smi">myStore</span>)]);
</code></pre><p>When executor is <a href="#settle-an-executor">settled</a>, <a href="#clear-an-executor">cleared</a>, <a href="#invalidate-results">invalidated</a> or annotated then the plugin calls the <a href="https://smikhalevski.github.io/react-executor/interfaces/plugin_syncExternalStore.ExternalStore.html#set"><code>ExternalStore.set</code> <sup>↗</sup></a> method on the store.</p><p>When executor is <a href="#detach-an-executor">detached</a> then the plugin calls the <a href="https://smikhalevski.github.io/react-executor/interfaces/plugin_syncExternalStore.ExternalStore.html#delete"><code>ExternalStore.delete</code> <sup>↗</sup></a> method on the store.</p><p>Prefer <a href="#syncbrowserstorage"><code>syncBrowserStorage</code></a> if you want to persist the executor state in a <code>localStorage</code> or <code>sessionStorage</code>.</p><h1 id="react-integration"><a class="markdown-permalink" href="#react-integration"><span class="icon icon-link"></span></a>React integration</h1><p>In the basic scenario, to use executors in your React app, you don't need any additional configuration, just use the <a href="https://smikhalevski.github.io/react-executor/functions/react-executor.useExecutor.html"><code>useExecutor</code> <sup>↗</sup></a> hook right away:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">useExecutor</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor<span class="pl-pds">'</span></span>;
...
<span class="pl-k">import</span> { <span class="pl-smi">hydrateExecutorManager</span>, <span class="pl-smi">ExecutorManager</span>, <span class="pl-smi">ExecutorManagerProvider</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor<span class="pl-pds">'</span></span>;

<span class="pl-k">const</span> <span class="pl-c1">manager</span> <span class="pl-k">=</span> <span class="pl-k">new</span> <span class="pl-en">ExecutorManager</span>();

<span class="pl-c">// 🟡 Hydrates executors on the client with the server data</span>
<span class="pl-en">hydrateExecutorManager</span>(<span class="pl-smi">manager</span>);
...
</code></pre><p>Here, <code>App</code> is the component that renders your application. Inside the <code>App</code> you can use <code>useExecutor</code> and <a href="#suspense"><code>useExecutorSuspence</code></a> to load your data.</p><p><a href="https://smikhalevski.github.io/react-executor/functions/react-executor.hydrateExecutorManager.html"><code>hydrateExecutorManager</code> <sup>↗</sup></a> must be called only once on the client-side with the manager that would receive the dehydrated state from the server.</p><p>On the server-side, you can either render your app contents <a href="#render-to-string">as a string</a> and send it to the client in one go, or <a href="#streaming-ssr">stream the contents</a>.</p><h2 id="render-to-string"><a class="markdown-permalink" href="#render-to-string"><span class="icon icon-link"></span></a>Render to string</h2><p>Use <a href="https://smikhalevski.github.io/react-executor/classes/ssr.SSRExecutorManager.html"><code>SSRExecutorManager</code> <sup>↗</sup></a> to render your app as an HTML string:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">createServer</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>node:http<span class="pl-pds">'</span></span>;
...
<span class="pl-k">const</span> <span class="pl-c1">server</span> <span class="pl-k">=</span> <span class="pl-en">createServer</span>(<span class="pl-k">async</span> (<span class="pl-v">request</span>, <span class="pl-v">response</span>) <span class="pl-k">=></span> {
  ...
  <span class="pl-c">// 2️⃣ Re-render until there are no more changes</span>
  ...
  <span class="pl-smi">html</span> <span class="pl-k">=</span> <span class="pl-smi">html</span>.<span class="pl-c1">replace</span>(<span class="pl-s"><span class="pl-pds">'</span>&#x3C;/body><span class="pl-pds">'</span></span>, <span class="pl-smi">manager</span>.<span class="pl-en">nextHydrationChunk</span>() <span class="pl-k">+</span> <span class="pl-s"><span class="pl-pds">'</span>&#x3C;/body><span class="pl-pds">'</span></span>);
...
<span class="pl-k">const</span> <span class="pl-c1">server</span> <span class="pl-k">=</span> <span class="pl-en">createServer</span>(<span class="pl-k">async</span> (<span class="pl-v">request</span>, <span class="pl-v">response</span>) <span class="pl-k">=></span> {
  ...
  <span class="pl-k">const</span> <span class="pl-c1">hydrator</span> <span class="pl-k">=</span> <span class="pl-k">new</span> <span class="pl-en">TransformStream</span>({
    <span class="pl-en">transform</span>(<span class="pl-v">chunk</span>, <span class="pl-v">controller</span>) {
      ...
      <span class="pl-smi">controller</span>.<span class="pl-en">enqueue</span>(<span class="pl-smi">manager</span>.<span class="pl-en">nextHydrationChunk</span>());
    },
  });
...
<span class="pl-k">function</span> <span class="pl-en">Hello</span>() {
...
</code></pre><p>If the <code>App</code> is rendered in streaming mode, it would first show "Loading" and after the executor is settled, it would update to "Hello, Paul!". In the meantime <code>helloExecutor</code> on the client would be hydrated with the data from the server.</p><h2 id="state-serialization"><a class="markdown-permalink" href="#state-serialization"><span class="icon icon-link"></span></a>State serialization</h2><p>By default, an executor state is serialized using <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON"><code>JSON</code> <sup>↗</sup></a> that has quite a few limitations. If your executor stores a value that may contain circular references, or non-serializable data like <code>BigInt</code>, then a custom state serializer must be provided.</p><p>On the server, pass a <a href="https://smikhalevski.github.io/react-executor/interfaces/ssr.SSRExecutorManagerOptions.html#serializer"><code>serializer</code> <sup>↗</sup></a> option to <a href="#render-to-string"><code>SSRExecutorManager</code></a>:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">SSRExecutorManager</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor/ssr<span class="pl-pds">'</span></span>;
...
<span class="pl-k">const</span> <span class="pl-c1">manager</span> <span class="pl-k">=</span> <span class="pl-k">new</span> <span class="pl-en">SSRExecutorManager</span>({ serializer: <span class="pl-smi">JSONMarshal</span> });
</code></pre><p>On the client, pass <em>the same</em> <a href="https://smikhalevski.github.io/react-executor/interfaces/react-executor.HydrateExecutorManagerOptions.html#serializer"><code>serializer</code> <sup>↗</sup></a> to <code>hydrateExecutorManager</code>:</p><pre><code class="language-tsx"><span class="pl-k">import</span> <span class="pl-smi">React</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> { <span class="pl-smi">hydrateRoot</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-dom/client<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> { <span class="pl-smi">hydrateExecutorManager</span>, <span class="pl-smi">ExecutorManager</span>, <span class="pl-smi">ExecutorManagerProvider</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> <span class="pl-smi">JSONMarshal</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>json-marshal<span class="pl-pds">'</span></span>;
...
<span class="pl-en">hydrateExecutorManager</span>(<span class="pl-smi">manager</span>, { serializer: <span class="pl-smi">JSONMarshal</span> });
...
<span class="pl-k">const</span> <span class="pl-c1">manager</span> <span class="pl-k">=</span> <span class="pl-k">typeof</span> <span class="pl-c1">window</span> <span class="pl-k">!==</span> <span class="pl-s"><span class="pl-pds">'</span>undefined<span class="pl-pds">'</span></span> <span class="pl-k">?</span> <span class="pl-en">hydrateExecutorManager</span>(<span class="pl-k">new</span> <span class="pl-en">ExecutorManager</span>()) <span class="pl-k">:</span> <span class="pl-c1">undefined</span>;
...
</code></pre><h2 id="storage-state-versioning"><a class="markdown-permalink" href="#storage-state-versioning"><span class="icon icon-link"></span></a>Storage state versioning</h2><p>You can store an executor state in a <code>localStorage</code> using the <a href="#syncbrowserstorage"><code>syncBrowserStorage</code></a> plugin:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">useExecutor</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> <span class="pl-smi">syncBrowserStorage</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-executor/plugin/syncBrowserStorage<span class="pl-pds">'</span></span>;

<span class="pl-k">const</span> <span class="pl-c1">playerExecutor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>player<span class="pl-pds">'</span></span>, { health: <span class="pl-s"><span class="pl-pds">'</span>50%<span class="pl-pds">'</span></span> }, [<span class="pl-en">syncBrowserStorage</span>()]);
<span class="pl-c">// ⮕ Executor&#x3C;{ health: string }></span>
</code></pre><p>But what if over time you'd like to change the structure of the value stored in the <code>playerExecutor</code>? For example, make <code>health</code> property a number:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">playerExecutor</span> <span class="pl-k">=</span> <span class="pl-en">useExecutor</span>(<span class="pl-s"><span class="pl-pds">'</span>player<span class="pl-pds">'</span></span>, { health: <span class="pl-c1">0.5</span> }, [<span class="pl-en">syncBrowserStorage</span>()]);
...
</code></pre>`};function u(){return s.createElement(e,{logo:s.createElement("div",{style:l(p,a),className:n.Logo,title:"React Executor"}),readme:c})}export{u as default};

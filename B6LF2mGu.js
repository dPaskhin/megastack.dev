...
</code></pre><p>Now we need a <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.Router.html"><code>Router</code> <sup>↗</sup></a> that would handle the navigation:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">Router</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-corsair<span class="pl-pds">'</span></span>;
...
</code></pre><p>To let the router know what route to render, call <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.Router.html#navigate"><code>navigate</code> <sup>↗</sup></a>:</p><pre><code class="language-ts"><span class="pl-smi">router</span>.<span class="pl-c1">navigate</span>(<span class="pl-smi">helloRoute</span>);
</code></pre><p>Use <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.RouterProvider.html"><code>&#x3C;RouterProvider></code> <sup>↗</sup></a> to render the router:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">RouterProvider</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-corsair<span class="pl-pds">'</span></span>;
...
<span class="pl-k">const</span> <span class="pl-c1">helloRoute</span> <span class="pl-k">=</span> <span class="pl-en">createRoute</span>(<span class="pl-s"><span class="pl-pds">'</span>/hello<span class="pl-pds">'</span></span>, <span class="pl-smi">HelloPage</span>);
</code></pre><p>In this example we used a shorthand signature of the <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.createRoute.html"><code>createRoute</code> <sup>↗</sup></a> function. You can also use a <a href="https://smikhalevski.github.io/react-corsair/interfaces/react-corsair.RouteOptions.html">route options object <sup>↗</sup></a>:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">helloRoute</span> <span class="pl-k">=</span> <span class="pl-en">createRoute</span>({
...
</code></pre><p>To trigger navigation from inside a component, use the <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.useRouter.html"><code>useRouter</code> <sup>↗</sup></a> hook:</p><pre><code class="language-tsx"><span class="pl-k">function</span> <span class="pl-en">AnotherPage</span>() {
...
</code></pre><p>Router cannot create a location for a parameterized route by itself, because it doesn't know the required param values. So here's where <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.Route.html#getlocation"><code>getLocation</code> <sup>↗</sup></a> comes handy:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">productLocation</span> <span class="pl-k">=</span> <span class="pl-smi">productRoute</span>.<span class="pl-en">getLocation</span>({ sku: <span class="pl-c1">42</span> });
...
</code></pre><p>To access params from a component use the <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.useRoute.html"><code>useRoute</code> <sup>↗</sup></a> hook:</p><pre><code class="language-tsx"><span class="pl-k">function</span> <span class="pl-en">ProductPage</span>() {
...
</code></pre><p>To use <code>:</code> as a character in a pathname pattern, replace it with an <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/encodeURIComponent">encoded <sup>↗</sup></a> representation <code>%3A</code>:</p><pre><code class="language-ts"><span class="pl-en">createRoute</span>(<span class="pl-s"><span class="pl-pds">'</span>/foo%3Abar<span class="pl-pds">'</span></span>);
</code></pre><h2 id="outlets"><a class="markdown-permalink" href="#outlets"><span class="icon icon-link"></span></a>Outlets</h2><p>Route components are rendered inside an <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.Outlet.html"><code>&#x3C;Outlet></code> <sup>↗</sup></a>. If you don't provide children to <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.RouterProvider.html"><code>&#x3C;RouterProvider></code> <sup>↗</sup></a> then it would implicitly render an <code>&#x3C;Outlet></code>:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">Router</span>, <span class="pl-smi">RouterProvider</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-corsair<span class="pl-pds">'</span></span>;
...
</code></pre><p>Routes are <a href="#outlets">rendered inside outlets</a>, so <code>ParentPage</code> should render an <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.Outlet.html"><code>&#x3C;Outlet></code> <sup>↗</sup></a> to give place for a <code>ChildPage</code>:</p><pre><code class="language-tsx"><span class="pl-k">function</span> <span class="pl-en">ParentPage</span>() {
  <span class="pl-k">return</span> (
...
</code></pre><p>To allow router navigation to <code>childRoute</code> it should be listed among <a href="https://smikhalevski.github.io/react-corsair/interfaces/react-corsair.RouterOptions.html#routes"><code>routes</code> <sup>↗</sup></a>:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">router</span> <span class="pl-k">=</span> <span class="pl-k">new</span> <span class="pl-en">Router</span>({ routes: [<span class="pl-smi">childRoute</span>] });
...
</code></pre><p>When router is navigated to the <code>userRoute</code>, a module that contains <code>&#x3C;UserPage></code> is loaded and rendered. The loaded component is cached, so next time the <code>userRoute</code> is matched, <code>&#x3C;UserPage></code> would be rendered instantly.</p><p>A promise is thrown if the <code>lazyComponent</code> isn't loaded yet. You can manually wrap <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.RouterProvider.html"><code>&#x3C;RouterProvider></code> <sup>↗</sup></a> in a custom <code>&#x3C;Suspense></code> boundary to catch it and render a fallback:</p><pre><code class="language-tsx"><span class="pl-k">function</span> <span class="pl-en">LoadingIndicator</span>() {
...
</code></pre><p><a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.Route.html#dataloader"><code>dataLoader</code> <sup>↗</sup></a> is called every time the router is navigated to <code>productRoute</code>. While data is being loaded, the <code>&#x3C;LoadingIndicator></code> is rendered instead of the <code>&#x3C;ProductPage></code>.</p><p>You can access the loaded data in your route component using the <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.useRoute.html"><code>useRoute</code> <sup>↗</sup></a> hook:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">ProductPage</span>() {
...
</code></pre><h2 id="error-boundaries"><a class="markdown-permalink" href="#error-boundaries"><span class="icon icon-link"></span></a>Error boundaries</h2><p>Each route is rendered in its own <a href="https://react.dev/reference/react/Component#catching-rendering-errors-with-an-error-boundary">error boundary <sup>↗</sup></a>. If an error occurs during route component rendering or <a href="#data-loading">data loading</a>, then an <a href="https://smikhalevski.github.io/react-corsair/interfaces/react-corsair.RouteOptions.html#errorcomponent"><code>errorComponent</code> <sup>↗</sup></a> is rendered as a fallback:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">ProductsPage</span>() {
...
</code></pre><p>Some errors are recoverable and only require a route data or component to be <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.RouteController.html#reload">reloaded <sup>↗</sup></a>:</p><pre><code class="language-tsx"><span class="pl-k">function</span> <span class="pl-en">ErrorDetails</span>() {
...
</code></pre><p><code>notFound</code> throws the <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.NotFoundError.html"><code>NotFoundError</code> <sup>↗</sup></a> symbol and aborts further rendering of the route component. The <code>&#x3C;Outlet></code> catches <code>NotFoundError</code> and renders a <a href="https://smikhalevski.github.io/react-corsair/interfaces/react-corsair.RouteOptions.html#notfoundcomponent"><code>notFoundComponent</code> <sup>↗</sup></a> as a fallback:</p><pre><code class="language-ts"><span class="pl-k">function</span> <span class="pl-en">ProductNotFound</span>() {
...
</code></pre><p>Router would render a <a href="https://smikhalevski.github.io/react-corsair/interfaces/react-corsair.RouteOptions.html#loadingcomponent"><code>loadingComponent</code> <sup>↗</sup></a> when <code>redirect</code> is called during a data loading or during rendering.</p><p><code>redirect</code> accepts routes, <a href="https://smikhalevski.github.io/react-corsair/interfaces/react-corsair.Location.html">locations <sup>↗</sup></a>, and URL strings as an argument.</p><p>Rect Corsair doesn't have a default behavior for redirects. Use a router event listener to handle redirects:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">router</span> <span class="pl-k">=</span> <span class="pl-k">new</span> <span class="pl-en">Router</span>({ routes: [<span class="pl-smi">adminRoute</span>] });
...
</code></pre><p>If you want the browser history to drive your redirects, see the <a href="#history">History</a> section.</p><h2 id="prefetching"><a class="markdown-permalink" href="#prefetching"><span class="icon icon-link"></span></a>Prefetching</h2><p>Sometimes you know ahead of time that a user would visit a particular route, and you may want to prefetch the component and <a href="#data-loading">related data</a> so the navigation is instant.</p><p>To do this, call the <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.Router.html#prefetch"><code>Router.prefetch</code> <sup>↗</sup></a> method and provide a route or a location to prefetch. Router would load required <a href="#code-splitting">components</a> and trigger <a href="#data-loading">data loaders</a>:</p><pre><code class="language-ts"><span class="pl-smi">router</span>.<span class="pl-en">prefetch</span>(<span class="pl-smi">productRoute</span>);
</code></pre><p>If a route requires params, use <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.Route.html#getlocation"><code>getLocation</code> <sup>↗</sup></a> to create a prefetched location:</p><pre><code class="language-ts"><span class="pl-smi">router</span>.<span class="pl-en">prefetch</span>(<span class="pl-smi">user</span>.<span class="pl-en">getLocation</span>({ userId: <span class="pl-c1">42</span> }));
</code></pre><p>Use <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.Prefetch.html"><code>Prefetch</code> <sup>↗</sup></a> component for a more declarative route prefetching:</p><pre><code class="language-tsx">&#x3C;<span class="pl-c1">Prefetch</span> <span class="pl-e">to</span><span class="pl-k">=</span><span class="pl-pse">{</span><span class="pl-smi">productRoute</span><span class="pl-pse">}</span> />
</code></pre><p>Or <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.usePrefetch.html"><code>usePrefetch</code> <sup>↗</sup></a> hook:</p><pre><code class="language-tsx"><span class="pl-en">usePrefetch</span>(<span class="pl-smi">productRoute</span>);
...
</code></pre><p>When button is hovered, <code>usePrefetch</code> would start prefetching <code>productRoute</code>.</p><p>React Corsair provides two prefetch triggers:</p><ul><li><p><a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.createHoveredPrefetchTrigger.html"><code>createHoveredPrefetchTrigger</code> <sup>↗</sup></a> Creates a trigger that start prefetching when an element is hovered.</p></li><li><p><a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.createVisiblePrefetchTrigger.html"><code>createVisiblePrefetchTrigger</code> <sup>↗</sup></a> Creates a trigger that start prefetching when an element is at least 50% visible on the screen.</p></li></ul><p>Create a custom prefetch trigger:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">usePrefetch</span>, <span class="pl-smi">PrefetchTrigger</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-corsair<span class="pl-pds">'</span></span>;
...
</code></pre><p>Now when user clicks on <em>Go to product</em>, the browser location changes to <code>/product/42</code> and <code>&#x3C;ShopPage></code> is re-rendered. <code>productRouteController</code> would contain a <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.RouteController.html">route controller <sup>↗</sup></a> for <code>productRoute</code>. This controller can be then rendered using the <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.RouteOutlet.html"><code>&#x3C;RouteOutlet></code> <sup>↗</sup></a>.</p><p>If a user clicks the <em>Reload</em> button in the browser, a <code>&#x3C;ProductPage></code> would be rendered because it matches <code>/product/42</code>.</p><p>You can render <code>&#x3C;RouteOutlet></code> in a popup to show the product preview, allowing user not to loose the context of the shop feed.</p><p>Use <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.Router.html#cancelinterception"><code>cancelInterception</code> <sup>↗</sup></a> method to render the intercepted route in a router <code>&#x3C;Outlet></code>:</p><pre><code class="language-ts"><span class="pl-smi">router</span>.<span class="pl-en">cancelInterception</span>();
...
</code></pre><p>There are three types of history adapters that you can leverage:</p><ul><li><p><a href="https://smikhalevski.github.io/react-corsair/functions/history.createBrowserHistory.html"><code>createBrowserHistory</code> <sup>↗</sup></a> is a DOM-specific history adapter that uses HTML5 history API.</p></li><li><p><a href="https://smikhalevski.github.io/react-corsair/functions/history.createHashBrowserHistory.html"><code>createHashBrowserHistory</code> <sup>↗</sup></a> is a DOM-specific history adapter that uses HTML5 history API and stores location in a <a href="https://developer.mozilla.org/en-US/docs/Web/API/URL/hash">URL hash <sup>↗</sup></a>. This is useful if your server doesn't support history fallback, or if you're shipping an HTML file.</p></li><li><p><a href="https://smikhalevski.github.io/react-corsair/functions/history.createMemoryHistory.html"><code>createMemoryHistory</code> <sup>↗</sup></a> is an in-memory history adapter, useful in testing and non-DOM environments like SSR.</p></li></ul><h2 id="route-urls"><a class="markdown-permalink" href="#route-urls"><span class="icon icon-link"></span></a>Route URLs</h2><p>History converts locations to URLs and vice-versa:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">helloRoute</span> <span class="pl-k">=</span> <span class="pl-en">createRoute</span>&#x3C;{ <span class="pl-v">name</span><span class="pl-k">?:</span> <span class="pl-c1">string</span> }>(<span class="pl-s"><span class="pl-pds">'</span>/hello<span class="pl-pds">'</span></span>);
...
</code></pre><p>Use <a href="https://smikhalevski.github.io/react-corsair/interfaces/history.HistoryOptions.html#basepathname"><code>basePathname</code> <sup>↗</sup></a> to set the base pathname for the entire history. URLs produced by history would share the base pathname:</p><pre><code class="language-ts"><span class="pl-en">createBrowserHistory</span>({ basePathname: <span class="pl-s"><span class="pl-pds">'</span>/suuuper<span class="pl-pds">'</span></span> }).<span class="pl-en">toURL</span>(<span class="pl-smi">helloRoute</span>);
...
</code></pre><p>URLs can be passed to <a href="https://smikhalevski.github.io/react-corsair/interfaces/history.History.html#push"><code>push</code> <sup>↗</sup></a> and <a href="https://smikhalevski.github.io/react-corsair/interfaces/history.History.html#replace"><code>replace</code> <sup>↗</sup></a> methods:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">history</span> <span class="pl-k">=</span> <span class="pl-en">createBrowserHistory</span>({ basePathname: <span class="pl-s"><span class="pl-pds">'</span>/suuuper<span class="pl-pds">'</span></span> });
...
</code></pre><p>By default, history serializes <a href="https://smikhalevski.github.io/react-corsair/interfaces/react-corsair.Location.html#searchparams">search params <sup>↗</sup></a> with <a href="https://smikhalevski.github.io/react-corsair/variables/history.jsonSearchParamsSerializer.html"><code>jsonSearchParamsSerializer</code> <sup>↗</sup></a> which serializes individual params with <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON"><code>JSON</code> <sup>↗</sup></a>:</p><pre><code class="language-ts"><span class="pl-k">interface</span> <span class="pl-en">ShopParams</span> {
...
</code></pre><p><code>jsonSearchParamsSerializer</code> allows you to store complex data structures in a URL.</p><p>You can create <a href="https://smikhalevski.github.io/react-corsair/interfaces/history.HistoryOptions.html#searchparamsserializer">a custom search params serializer <sup>↗</sup></a> and provide it to a history. Here's how to create a basic serializer that uses <a href="https://developer.mozilla.org/en-US/docs/Web/API/URLSearchParams"><code>URLSearchParams</code> <sup>↗</sup></a>:</p><pre><code class="language-ts"><span class="pl-en">createBrowserHistory</span>({
...
</code></pre><h2 id="links"><a class="markdown-permalink" href="#links"><span class="icon icon-link"></span></a>Links</h2><p>Inside components use <a href="https://smikhalevski.github.io/react-corsair/functions/history.Link.html"><code>&#x3C;Link></code> <sup>↗</sup></a> for navigation:</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">Link</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-corsair/history<span class="pl-pds">'</span></span>;
...
</code></pre><h1 id="server-side-rendering"><a class="markdown-permalink" href="#server-side-rendering"><span class="icon icon-link"></span></a>Server-side rendering</h1><p>Routes can be rendered on the server side and then hydrated on the client side.</p><p>To enable hydration on the client, create a <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.Router.html"><code>Router</code> <sup>↗</sup></a> and call <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.hydrateRouter.html"><code>hydrateRouter</code> <sup>↗</sup></a> instead of <a href="https://smikhalevski.github.io/react-corsair/classes/react-corsair.Router.html#navigate"><code>Router.navigate</code> <sup>↗</sup></a>:</p><pre><code class="language-tsx"><span class="pl-k">import</span> <span class="pl-smi">React</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react<span class="pl-pds">'</span></span>;
...
</code></pre><p>A new router and a new history must be created for each request, so the results that are stored in router are served in response to a particular request.</p><p><a href="https://smikhalevski.github.io/react-corsair/classes/ssr.SSRRouter.html#haschanges"><code>hasChanges</code> <sup>↗</sup></a> would resolve with <code>true</code> if state of some routes have changed during rendering.</p><p>The hydration chunk returned by <a href="https://smikhalevski.github.io/react-corsair/classes/ssr.SSRRouter.html#nexthydrationchunk"><code>nextHydrationChunk</code> <sup>↗</sup></a> contains the <code>&#x3C;script></code> tag that hydrates the router for which <a href="https://smikhalevski.github.io/react-corsair/functions/react-corsair.hydrateRouter.html"><code>hydrateRouter</code> <sup>↗</sup></a> is invoked on the client side.</p><h2 id="streaming-ssr"><a class="markdown-permalink" href="#streaming-ssr"><span class="icon icon-link"></span></a>Streaming SSR</h2><p>React can stream parts of your app while it is being rendered. You can inject React Corsair hydration chunks into the React stream.</p><pre><code class="language-tsx"><span class="pl-k">import</span> { <span class="pl-smi">createServer</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>node:http<span class="pl-pds">'</span></span>;
...
<span class="pl-smi">server</span>.<span class="pl-en">listen</span>(<span class="pl-c1">8080</span>);
</code></pre><p><code>hydrator</code> injects React Executor hydration chunks into the React stream.</p><h2 id="state-serialization"><a class="markdown-permalink" href="#state-serialization"><span class="icon icon-link"></span></a>State serialization</h2><p>By default, route state is serialized using <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify"><code>JSON</code> <sup>↗</sup></a> which has quite a few limitations. If your route <a href="#data-loading">loads data</a> that may contain circular references, or non-serializable data like <code>BigInt</code>, use a custom state serialization.</p><p>On the server, pass a <a href="https://smikhalevski.github.io/react-corsair/interfaces/ssr.SSRRouterOptions.html#serializer"><code>serializer</code> <sup>↗</sup></a> option to <a href="#render-to-string"><code>SSRRouter</code></a>:</p><pre><code class="language-ts"><span class="pl-k">import</span> { <span class="pl-smi">SSRRouter</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-corsair/ssr<span class="pl-pds">'</span></span>;
...
</code></pre><p>On the client, pass <em>the same</em> <a href="https://smikhalevski.github.io/react-corsair/interfaces/react-corsair.HydrateRouterOptions.html#serializer"><code>serializer</code> <sup>↗</sup></a> option to <code>hydrateRouter</code>:</p><pre><code class="language-tsx"><span class="pl-k">import</span> <span class="pl-smi">React</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> { <span class="pl-smi">hydrateRoot</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-dom/client<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> { <span class="pl-smi">createBrowserHistory</span>, <span class="pl-smi">HistoryProvider</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-router/history<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> { <span class="pl-smi">hydrateRouter</span>, <span class="pl-smi">Router</span>, <span class="pl-smi">RouterProvider</span> } <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>react-router<span class="pl-pds">'</span></span>;
<span class="pl-k">import</span> <span class="pl-smi">JSONMarshal</span> <span class="pl-k">from</span> <span class="pl-s"><span class="pl-pds">'</span>json-marshal<span class="pl-pds">'</span></span>;
...
</code></pre><div class="markdown-alert markdown-alert-tip"><p class="markdown-alert-title"><svg class="octicon octicon-light-bulb mr-2" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="M8 1.5c-2.363 0-4 1.69-4 3.75 0 .984.424 1.625.984 2.304l.214.253c.223.264.47.556.673.848.284.411.537.896.621 1.49a.75.75 0 0 1-1.484.211c-.04-.282-.163-.547-.37-.847a8.456 8.456 0 0 0-.542-.68c-.084-.1-.173-.205-.268-.32C3.201 7.75 2.5 6.766 2.5 5.25 2.5 2.31 4.863 0 8 0s5.5 2.31 5.5 5.25c0 1.516-.701 2.5-1.328 3.259-.095.115-.184.22-.268.319-.207.245-.383.453-.541.681-.208.3-.33.565-.37.847a.751.751 0 0 1-1.485-.212c.084-.593.337-1.078.621-1.489.203-.292.45-.584.673-.848.075-.088.147-.173.213-.253.561-.679.985-1.32.985-2.304 0-2.06-1.637-3.75-4-3.75ZM5.75 12h4.5a.75.75 0 0 1 0 1.5h-4.5a.75.75 0 0 1 0-1.5ZM6 15.25a.75.75 0 0 1 .75-.75h2.5a.75.75 0 0 1 0 1.5h-2.5a.75.75 0 0 1-.75-.75Z"></path></svg>Tip</p><p>Read more about <a href="https://github.com/smikhalevski/json-marshal#readme">JSON Marshal <sup>↗</sup></a>, it can stringify and parse any data structure.</p></div><h2 id="content-security-policy-support"><a class="markdown-permalink" href="#content-security-policy-support"><span class="icon icon-link"></span></a>Content-Security-Policy support</h2><p>By default, <a href="https://smikhalevski.github.io/react-corsair/classes/ssr.SSRRouter.html#nexthydrationchunk"><code>nextHydrationChunk</code> <sup>↗</sup></a> renders an inline <code>&#x3C;script></code> tag without any attributes. To enable the support of the <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/script-src"><code>script-src</code> <sup>↗</sup></a> directive of the <code>Content-Security-Policy</code> header, provide the <a href="https://smikhalevski.github.io/react-corsair/interfaces/ssr.SSRRouterOptions.html#nonce"><code>nonce</code> <sup>↗</sup></a> option to <code>SSRRouter</code> or any of its subclasses:</p><pre><code class="language-ts"><span class="pl-k">const</span> <span class="pl-c1">router</span> <span class="pl-k">=</span> <span class="pl-k">new</span> <span class="pl-en">SSRRouter</span>({
  routes: [<span class="pl-smi">helloRoute</span>],
